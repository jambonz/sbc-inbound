...
module.exports = function(srf, logger) {
  ...
  const handleSipRec = async(req, res, next) => {
    ...
    if (Array.isArray(req.payload) && req.payload.length > 1) {
      ...
      const endPos = xml.indexOf('</recording>');
      const xmlString = endPos !== -1 ? xml.substring(0, endPos + 12) : xml;
      logger.debug({callId, xmlString}, 'incoming call with SIPREC body');
      req.locals = {...req.locals, sdp, siprec: true, xml: xmlString};
    }
