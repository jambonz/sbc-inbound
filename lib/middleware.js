...
module.exports = function(srf, logger) {
  ...
  const {stats, writeCdrs, lookupAuthCarriersForAccountAndSP} = srf.locals;
  ...
  const identifyAccount = async(req, res, next) => {
    try {
      ...
      if (fromCarrier) {
        ...
        if (auth_trunks?.length) {
          logger.info({callId, auth_trunks}, 'identifyAccount: found auth trunks for account');
        }
  ...
  const identifyAuthTrunk = async(req, res, next) => {
    try {
      const {authorization} = req.locals;
      if (authorization) {
        ...
        if (grant && grant.status === 'ok' && grant.auth_trunk) {
          ...
          req.locals = {
            ...
            application_sid: grant.auth_trunk.application_sid,
          };
          logger.info({callId: req.locals.callId, auth_trunk: grant.auth_trunk.name},
            'identifyAuthTrunk: authenticated call for auth_trunk');
        }
      }
      next();
    } catch (err) {
