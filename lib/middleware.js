  ...
  const createRootSpan = (req, res, next) => {
    ...
    const getAttributes = () => {
      if (req instanceof Dialog) {
        const {sip: {callId}, callSid, linkedSpanId} = req;
        return {
          linkedSpanId,
          callId,
          callSid,
        ...
        const {locals: {callSid, application_sid}} = req;
        return {
          callSid,
    ...
    const {callSid, ...attributes} = getAttributes();
    const rootSpan = new RootSpan('sbc-incoming-call', callSid, attributes, tracer, logger);

    const traceId = rootSpan.traceId;
    const {callId, accountSid} = attributes;
