...
const {Dialog, parseUri} = require('drachtio-srf');
...
module.exports = function(srf, logger) {
  ...
  const createRootSpan = (req, res, next) => {

    const getAttributes = () => {
      if (req instanceof Dialog) {
        const {sip: {callId}} = req;
        return {callId};
      } else {
        return {callId: req.get('Call-ID')};
      }
    };

    const {tracer} = req.srf.locals.otel;
    const {callId, ...attributes} = getAttributes();
    const newTraceId = req.get('X-Trace-ID') || callId.replaceAll('-', '').substring(0, 32);
    const rootSpan = new RootSpan('sbc-incoming-call', newTraceId, attributes, tracer, logger);
  ...
  const identifyAccount = async(req, res, next) => {
    const {siprec, callId, rootSpan} = req.locals;
    const childSpan = rootSpan.startChildSpan('identifyAccount');
    try {
      ...
      if (fromCarrier) {
        if (error) {
          childSpan.setAttributes({'X-Reason': error});
          childSpan.endWithError('503 error');
          return res.send(503, {
            headers: {
              'X-Reason': error,
            },
        ...
        if (!gateway) {
          childSpan.endWithError('Number Not Provisioned');
        ...
        if (siprec) {
          if (!account.siprec_hook_sid) {
            childSpan.endWithError('SIPREC not provisioned');
        ...
        if (!app) {
          childSpan.endWithError('Teams no configured application');
        ...
        if (!account) {
          childSpan.setAttributes({realm: uri.host});
          childSpan.endWithError('No account for sip realm');
          stats.increment('sbc.terminations', ['sipStatus:404']);
          res.send(404);
          return req.srf.endSession(req);
        }
        ...
        if (process.env.SBC_ACCOUNT_SID && account.account_sid !== process.env.SBC_ACCOUNT_SID) {
          childSpan.setAttributes({
            ip: process.env.SBC_ACCOUNT_SID,
            account_sid: account.account_sid,
          });
          childSpan.endWithError('static IP for account_sid');
          logger.info(
      ...
      if (!req.locals.account.is_active) {
        childSpan.endWithError('Account exists but is inactive');
      ...
      if (req.locals.account.disable_cdrs) {
        childSpan.setAttributes({disableCDRs: 'true'});
      ...
      childSpan.endWithError('database error for inbound call');
