...
module.exports = function(srf, logger) {
  ...
  const checkLimits = async(req, res, next) => {
    ...
    res.once('end', ({status}) => {
      if (status > 200) {
        decrKey(key)
          ...
          .then((count) => writeCallCount({account_sid, service_provider_sid, calls_in_progress: count}))
    ...
    try {
      ...
      writeCallCount({account_sid, service_provider_sid, calls_in_progress: calls})
