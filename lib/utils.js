...
function makeRtpEngineOpts(req, srcIsUsingSrtp, dstIsUsingSrtp, teams = false) {
  ...
  /** By default use strict source to secure aganst RTPInject vuln, set env var to false to disable and use media handover instead */
  const strictSource = process.env.RTPENGINE_STRICT_SOURCE || true;
  dstOpts.flags.push(strictSource ? 'strict source' : 'media handover');
  srcOpts.flags.push(strictSource ? 'strict source' : 'media handover');
