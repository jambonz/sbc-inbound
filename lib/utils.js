...
function makeRtpEngineOpts(req, srcIsUsingSrtp, dstIsUsingSrtp, teams = false) {
   ...
   * set env var to false to disable and use media handover instead */
  const strictSource = process.env.RTPENGINE_DISABLE_STRICT_SOURCE || false;
  dstOpts.flags.push(strictSource ? 'media handover' : 'strict source');
  srcOpts.flags.push(strictSource ?  'media handover' : 'strict source');
