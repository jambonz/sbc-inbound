...
const makeAppCallCountKey = (sid) => `${sid}:incalls:app`;
...
const nudgeCallCounts = async(logger, sids, nudgeOperator, writers) => {
  ...
  try {
    ...
    logger.debug({calls, callsSP, callsApp}, 'call counts after adjustment');
    if (process.env.JAMBONES_TRACK_SP_CALLS) {
    ...
    if (process.env.JAMBONES_TRACK_APP_CALLS && application_sid) {
      writes.push(writeCallCountApp({application_sid, calls_in_progress: callsApp}));
    }
