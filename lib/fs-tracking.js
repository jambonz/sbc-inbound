...
assert.ok(!process.env.K8S || process.env.K8S_FEATURE_SERVER_SIP_SERVICE_NAME,
  'when running in Kubernetes, an env var K8S_FEATURE_SERVER_SIP_SERVICE_NAME is required');
...
module.exports = (srf, logger) => {
  ...
  return async() => {
    try {
      if (process.env.K8S) {
        return process.env.K8S_FEATURE_SERVER_SIP_SERVICE_NAME;
      }
      else {
