      ...
      return new Promise((resolve, reject) => {
        dns.resolve(`_sip._udp.${name}`, 'SRV', (err, results) => {
          if (err) {
            ...
            dns.resolve(name, 'A', (err, results) => {
              if (err) {
                logger.info({err}, `No A records found for ${name} either`);
                return reject(err);
              }
              logger.info({results}, 'found A records');
              resolve(name);
            });
          }
          logger.info({results}, 'found SRV records');
          resolve(name);
        });
      });
    }
