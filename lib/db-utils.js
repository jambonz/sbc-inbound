  ...
  const wasOriginatedFromCarrier = async(req) => {
    ...
    if (!isDotDecimal) {
      ...
      if (a.length) {
        ...
        const voip_carriers = gateways.map((gw) => {
          return {
        ...
        if (voip_carriers.length) {
          ...
          if (r.length > 1) {
            logger.info({r},
              'multiple carriers with the same gateway have the same number provisioned for the same account  -- cannot determine which to use');
            return {
              fromCarrier: true,
              error: 'Multiple carriers with the same gateway are attempting to route the same phone number for a this account'
            };
          }

          /* we have one or no routes for this phone number, carrier and account combination */
          const gateway = gateways.find((m) => m.voip_carrier_sid === r[0].voip_carrier_sid);
          return {
            fromCarrier: true,
            gateway: gateway,
