...
module.exports = (srf, logger) => {
   ...
   * @param {boolean} [appObject=false] - If true, returns the full application object;
   *                                     if false, returns only the application_sid
  ...
  const getApplicationForDidAndCarrier = async(req, voip_carrier_sid, appObject = false) => {
    const did = normalizeDID(req.calledNumber) || 'anonymous';

    try {
      /* straight DID match */
      const [r] = await pp.query(sqlQueryApplicationByDid, [did, voip_carrier_sid]);
      if (r.length) return appObject ? r[0] : r[0].application_sid;
      ...
      for (const pattern of patterns) {
        try {
          ...
          if (did.match(new RegExp(regexPattern))) {
            return appObject ? pattern : pattern.application_sid;
          }
        } catch (err) {
      ...
      if (a.length) {
        ...
        if (voip_carriers.length) {
          ...
          if (r.length === 0) {
            ...
            for (const match of voip_carriers) {
              const application = await getApplicationForDidAndCarrier(req, match.voip_carrier_sid, true);
              if (application) {
                logger.debug({application}, 'sip_realm looking up DID: found application with Regex or wildcard ');
                const gateway = gateways.find((m) => m.voip_carrier_sid === match.voip_carrier_sid);
                // the gateway may belong to "all accounts", so we need to assign the account_sid of the application
      ...
      if (0 === r.length) {
        ...
        for (const match of matches) {
          ...
          if (application) {
            ...
            // the gateway may belong to "all accounts", so we need to assign the account_sid of the application
            match.account_sid = match.account_sid ? match.account_sid : application.account_sid;
            return {
              fromCarrier: true,
              gateway: match,
              service_provider_sid: match.service_provider_sid,
              account_sid: match.account_sid,
              application_sid: application.application_sid,
