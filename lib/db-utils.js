...
const sqlQueryApplicationByDidAndCarrierList = `
SELECT * FROM phone_numbers 
WHERE number = ? 
    ...
    if (isDotDecimal) {
      if (process.env.JAMBONES_HOSTING) {
        ...
        if (matches) {
          ...
          const did = normalizeDID(req.calledNumber);
          logger.debug({did, vc_sids}, 'looking up DID');
          const [r] = await pp.query(
            `SELECT * FROM phone_numbers WHERE number = ${did} AND voip_carrier_sid IN (${vc_sids})`);
          if (0 === r.length) {
