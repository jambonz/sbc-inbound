...
const sqlSelectCarriersByVoipCarrierSids = `
SELECT sg.sip_gateway_sid, sg.voip_carrier_sid, vc.name, vc.service_provider_sid,
vc.account_sid, vc.application_sid, sg.inbound, sg.outbound, sg.is_active, sg.ipv4, sg.netmask, sg.pad_crypto
FROM sip_gateways sg, voip_carriers vc
...
module.exports = (srf, logger) => {
  ...
  /**
   * Retrieves full carrier and gateway details from MySQL for the given voip_carrier_sid array.
   * Returns carrier/gateway records matching the structure of other gateway queries.
   *
   * @param {Array<string>} voipCarrierSids - Array of voip_carrier_sid values
   * @returns {Promise<Array<Object>>} Array of gateway/carrier objects
   */
  const lookupCarrierDetailsByIds = async(voipCarrierSids) => {
    if (!voipCarrierSids || voipCarrierSids.length === 0) return [];

    try {
      const [r] = await pp.query(sqlSelectCarriersByVoipCarrierSids, [voipCarrierSids]);
      logger.debug({count: r.length, voipCarrierSids}, 'retrieved carrier details for ephemeral gateways');
      return r;
    } catch (err) {
      logger.error({err, voipCarrierSids}, 'Error retrieving carrier details by IDs');
      return [];
    }
  };
      ...
      if (a.length) {
        ...
        if (voip_carriers.length === 0 && gateways.length === 0) {
          ...
          if (ephemeralCarrierSids.length > 0) {
            const ephemeralGateways = await lookupCarrierDetailsByIds(ephemeralCarrierSids);
            // Create both voip_carriers and gateways arrays for ephemeral matches
