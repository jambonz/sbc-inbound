        ...
        if (voip_carriers.length) {
          ...
          if (r.length === 0) {
            ...
            for (const match of voip_carriers) {
              const application_sid = await getApplicationForDidAndCarrier(req, match.voip_carrier_sid);
              if (application_sid) {
                logger.debug({application_sid}, 'sip_realm looking up DID: found application with Regex or wildcard ');
    ...
    if (matches.length) {
      ...
      if (0 === r.length) {
        ...
        for (const match of matches) {
          const application_sid = await getApplicationForDidAndCarrier(req, match.voip_carrier_sid);
          if (application_sid) {
            logger.debug({application_sid}, 'looking up DID: found application with Regex or wildcard ');
            return {
              fromCarrier: true,
              gateway: match,
